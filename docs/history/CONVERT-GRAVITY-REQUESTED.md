...
Make every Convert peel/emit path land **honest CWL** (typed holes, no façades), prove cutover smokes, **pin `@agenticop-io/cwl@1.0.0`**, and reverse-home WebIR to chrysalis-cwl.
...
2. **Pin `1.0.0`** — Prefer `"@agenticop-io/cwl": "1.0.0"` from GitHub Packages; `file:../chrysalis-cwl/packages/cwl` OK during cutover. Refresh junctions / mirrors; no convert-only parser/print forks.
...
- [ ] Pin `@agenticop-io/cwl@1.0.0` (or `file:` ≡ `1.0.0`) + WebIR reverse-home checklist started
