...
**No invent queue.** Tip bumps only when siblings expose contract/sync gaps (as 1.0.17).
...
| **ALWAYS hub-ingest mirrors ≡ tip** (`setup:mirrors` / `sync:convert`) | Convert | **Local sync done** — Convert must commit — [`CONVERT-MIRRORS-REQUESTED.md`](./CONVERT-MIRRORS-REQUESTED.md) |
