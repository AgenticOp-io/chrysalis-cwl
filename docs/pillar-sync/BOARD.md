# Chrysalis sync BOARD (git SoR in CWL)

**Updated:** 2026-09-16 · tip **1.0.32** · RFC-0031 repeats + item fields  
**Protocol:** [`PROTOCOL.md`](./PROTOCOL.md) · [`COORDINATOR.md`](./COORDINATOR.md)  
**Queue:** [`../history/DNA-BUILD-NEXT.md`](../history/DNA-BUILD-NEXT.md)

```text
FLEET_MODE: on
CWL_FLEET_IDLE: no
DISPATCH: tip 1.0.32 land + Packages · Convert pin+repeat peel · Secure pin
CONVERT_NEXT: pin 1.0.32 · peel __cwl_html_repeat incl. member chains (golds 40-41)
SECURE_NEXT: pin 1.0.32 (was 1.0.28 ack)
```

## Tips / pins

| Surface | Value |
| --- | --- |
| **CWL tip** | **`1.0.32`** |
| Packages | **`@agenticop-io/cwl@1.0.32`** **live** |
| Convert | file: → bump acknowledgment to **1.0.32** (was 1.0.27) |
| Secure | file: → bump acknowledgment to **1.0.32** (was 1.0.28) |

## Latest SHAs

| Pillar | Branch | SHA | Note |
| --- | --- | --- | --- |
| **CWL** | `main` | `9d5d66b` | tip **1.0.32** · tag `cwl-v1.0.32` |
| **Convert** | `candidate/wptp-convert-orbit` | lag | tip ack **1.0.27** |
| **Secure** | `candidate/live-match-step4` | lag | tip ack **1.0.28** |

## Who builds next

| Priority | Owner | Work |
| --- | --- | --- |
| **P0** | Convert | Tip pin **1.0.32**; peel repeats incl. `data.member` item fields |
| **P0** | Secure | Tip pin **1.0.32**; DNA seed vs golds `40`–`41` |
| **P0** | Operator | Redeploy Cinderpath web once genome uses `repeat` |
| **done** | CWL | repeat gene + item fields + emit reverse |

## Honesty

Host keeps credential crypto and upstream bytes; WebSocket duplex stays a hole. No UA regex / Nest façades.
