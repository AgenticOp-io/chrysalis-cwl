...
**Package:** tip `@agenticop-io/cwl` / `@chrysalis/cwl` ≡ **1.0.34** (Packages lineage from `1.0.0`; siblings often `file:`)  
...
Install: Extensions → `Install from VSIX…` (or `code --install-extension dist-editors/cwl-lsp-1.0.34.vsix`).
...
1. Install `@agenticop-io/cwl@1.0.34` (or `file:` pin to this tip)
...
| Tip **1.0.34** deepen | Framework façades (Nest / LiveView / Flutter) |
