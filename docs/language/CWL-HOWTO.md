...
**Tip language:** **`1.0.31`** ([`LANGUAGE_VERSION.md`](../../LANGUAGE_VERSION.md))  
...
Published name: **`@agenticop-io/cwl`**. Tip version must match [`LANGUAGE_VERSION.md`](../../LANGUAGE_VERSION.md) (today **`1.0.31`**). A token with `read:packages` on org **AgenticOp-io** is required.
...
npm install @agenticop-io/cwl@1.0.31
```
...
```bash
code --install-extension dist-editors/cwl-lsp-1.0.31.vsix
```
...
# Package consumers
npm install @agenticop-io/cwl@1.0.31   # needs GH Packages auth
