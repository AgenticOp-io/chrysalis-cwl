...
**Tip language:** **`1.0.34`** ([`LANGUAGE_VERSION.md`](../../LANGUAGE_VERSION.md))  
...
Published name: **`@agenticop-io/cwl`**. Tip version must match [`LANGUAGE_VERSION.md`](../../LANGUAGE_VERSION.md) (today **`1.0.34`**). A token with `read:packages` on org **AgenticOp-io** is required.
...
npm install @agenticop-io/cwl@1.0.34
```
...
```bash
code --install-extension dist-editors/cwl-lsp-1.0.34.vsix
```
...
# Package consumers
npm install @agenticop-io/cwl@1.0.34   # needs GH Packages auth
