...
**Status:** accepted (2026-09-16) · deepen tip `1.0.40`  
**Tip:** `1.0.40` · golds `40-html-repeat`, `41-html-repeat-fields`, `47-html-repeat-if`, `48-html-repeat-else`
...
```text
data.call __cwl_html_repeat(param <collection>, html.template <item template>[, when][, empty])
```
...
`repeat` statements round-trip: CWL → WebIR → CWL reproduces the source line (including `if` and `else`).
When the iterable, item template, `when`, or `empty` cannot be recovered, emit keeps `cwl:emit:html-repeat`
