...
**Convert** translates the world’s stacks *into and out of* CWL.  
...
```text
                    ...
                    └────────────┬─────────────┘
                                 │
              produce / consume  │  semantics + fixtures + version
                                 │
              ┌──────────────────┼──────────────────┐
              ▼                  │                  ▼
     ┌────────────────┐          │         ┌────────────────┐
     │    Convert     │◄─────────┴────────►│  Secure/Helix  │
     │  translate only│   shared bar       │ DNA + CWL bridge│
     └────────────────┘                    └────────────────┘
```
...
| Product law | Easy to ship demo façades | Façades fail the language bar |
...
| --- | --- |
| RFC index 0001–0021 accepted and in-tree | Yes |
| Parse → print round-trip golds for core surfaces | Yes (`01`–`19`) |
...
| Ingest/fmt via WebIR without convert checkout | No — blocked on WebIR extract |
...
| DNA↔CWL bridge library in Secure | No (Secure later) |
